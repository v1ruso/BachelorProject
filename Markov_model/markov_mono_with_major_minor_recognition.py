...
import numpy as np
import glob
# import from key_detection algorithm
from key_measure import find_key
from key_measure import KEYS
from key_measure import is_major_key
...
pitches = {
    "major": np.zeros(NUMBER_OF_PITCHES, dtype=float),
    "minor": np.zeros(NUMBER_OF_PITCHES, dtype=float)
}
# first need to train on the prime dataset
major_notes = 0
minor_notes = 0
for filename in glob.glob(DATASET_FILEPATH + 'prime_midi/*.mid'):
    ...
    for instrument in input_data.instruments:
        if not instrument.is_drum:
            if ends_with_major:
                major_notes += len(instrument.notes)
            else:
